...
class FlashSaleController extends GetxController {
  ...
  Rxn<TimeSale> selectedFlash = Rxn<TimeSale>();
  ...
  void setSelectedFlash(TimeSale pro) => selectedFlash.value = pro;
    ...
    scrollController.addListener(() {
      if (scrollController.position.pixels ==
        ...
        if (!(isLoading.value)) {
          ...
          final lastD = products[selectedFlash.value!.id]?.last;
          debugPrint("******${lastD?.name}");
          getMoreProducts(
                  selectedFlash.value!.id, [lastD?.toJson()["dateTime"]])
